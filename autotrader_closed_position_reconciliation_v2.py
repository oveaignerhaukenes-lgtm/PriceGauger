...
def _account_currencies_v2(client) -> dict[str, str]:
...
def reconcile_closed_position_equity_once_v2(client=None) -> int:
    ...
    enrollment and an authoritative Saxo closed-position row carrying the exact
    `ClosingExternalReferenceId` generated by PriceGauger.
    """
    ...
    closed_payload = client._get("port/v1/closedpositions/me", params={"$top": 1000})
    realizations = closed_position_realizations_v2(closed_payload)
    currencies = _account_currencies_v2(client)
    booked = 0
    ...
    for attempt in attempts:
        ...
        if not matches:
            continue
        account_currency = currencies.get(str(attempt["account_id"]))
        if not account_currency:
            LOGGER.warning("P/L reconciliation blocked: account currency unavailable account=%s", attempt["account_id"])
            continue
