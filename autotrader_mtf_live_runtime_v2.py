...
from autotrader_strategy_catalog_v2 import MTF_LONG_FLAT_STRATEGY_V2
...
def ensure_mtf_live_schema_v2() -> None:
    ensure_autotrader_schema_v2()
    with connect() as db:
        db.execute(
            """
            CREATE TABLE IF NOT EXISTS pg_v2_autotrader_mtf_live_state (
        ...
        db.execute(
            """
            CREATE TABLE IF NOT EXISTS pg_v2_autotrader_mtf_live_events (
...
def load_mtf_live_state_v2(pilot_key: str) -> MtfLiveStateV2 | None:
    ...
    with connect() as db:
        row = db.execute(
            ...
            FROM pg_v2_autotrader_mtf_live_state
            WHERE pilot_key = ? AND strategy_key = ?
            """,
            (str(pilot_key), MTF_LONG_FLAT_STRATEGY_V2),
        ).fetchone()
    if row is None:
        return None
    item = _row_dict(row)
    return MtfLiveStateV2(
...
def _persist_state_v2(state: MtfLiveStateV2) -> None:
    with connect() as db:
        db.execute(
            """
            INSERT INTO pg_v2_autotrader_mtf_live_state(
...
def _exact_product_observation(
    ...
    if len(matches) > 1:
        raise RuntimeError("multiple Saxo positions match one MTF LIVE product")
...
def _cursor(state: MtfLiveStateV2, timeframe: int) -> datetime | None:
    return {
...
def _advance_state(
    ...
    *,
    ...
    if decision is not None and decision.event_type == EVENT_ENTRY_5M:
...
def _latest_work(
    ...
    *,
    ...
    """Return at most one current pair per timeframe; never replay an outage backlog."""
...
def _target_direction(decision: MtfDecisionV2) -> str | None:
    if decision.action == ACTION_WOULD_BUY:
        return "LONG"
    if decision.action in {ACTION_WOULD_EXIT, ACTION_WOULD_EXIT_REARM}:
        return "FLAT"
    if decision.action == ACTION_CONFIRMATION:
        return None
    raise ValueError(f"unsupported MTF action: {decision.action}")


def _request_action(observed_direction: str, desired_direction: str | None) -> str | None:
    if desired_direction is None:
        return None
    if observed_direction == "SHORT":
        raise ValueError("MTF long/flat cannot manage an observed SHORT exposure")
    if desired_direction == "LONG":
        return "OPEN" if observed_direction == "FLAT" else None
    if desired_direction == "FLAT":
        return "CLOSE" if observed_direction == "LONG" else None
    raise ValueError(f"unsupported MTF desired direction: {desired_direction}")


def _persist_event_and_request_v2(
    *,
    enrollment: StrategyEnrollmentV2,
    state_before: MtfLiveStateV2,
    ...
    if request_action == "OPEN" and float(budget_amount) <= 0:
        request_action = None
        outcome = "MTF_ENTRY_BLOCKED_EQUITY_EXHAUSTED"
    elif desired_direction is None:
        outcome = f"MTF_{decision.event_type}"
    elif request_action is None:
        outcome = "MTF_TARGET_ALREADY_OBSERVED"
    else:
        outcome = f"MTF_{decision.event_type}_REQUESTED"
    ...
    event_id = str(
        uuid5(
            NAMESPACE_URL,
            f"mtf-live-event|{enrollment.pilot_key}|{decision.event_type}|{current.closed_at.isoformat()}",
        )
    )
    evaluation_id = str(uuid5(NAMESPACE_URL, f"mtf-live-evaluation|{event_id}"))
    request_id = None
    if request_action is not None and signal is not None:
        request_id = str(uuid5(NAMESPACE_URL, f"mtf-live-execution|{event_id}|{request_action}|{desired_direction}"))

    with connect() as db:
        # Any fresh actionable MTF decision invalidates older unstarted authority.
        # SUBMITTING/accepted/uncertain requests are deliberately never rewritten.
        if signal is not None:
            db.execute(
                """
                UPDATE pg_v2_autotrader_execution_requests
                SET status = ?, block_reason = 'NEWER_MTF_SIGNAL', updated_at = now()
        ...
        db.execute(
            """
            INSERT INTO pg_v2_autotrader_mtf_live_events(
...
def _bootstrap_state_v2(
    *,
    ...
    if direction == "SHORT":
        raise ValueError("MTF long/flat cannot bootstrap from a SHORT position")
    state_name = STATE_CONFIRMED_30M if direction == "LONG" else STATE_FLAT
    state = MtfLiveStateV2(
    ...
    with connect() as db:
        evaluation_id = str(uuid5(NAMESPACE_URL, f"mtf-live-bootstrap|{enrollment.pilot_key}|{latest.isoformat()}"))
        db.execute(
            """
            INSERT INTO pg_v2_autotrader_strategy_evaluations(
                evaluation_id, pilot_key, strategy_key, latest_closed_bar_time,
...
def run_mtf_live_strategy_once_v2(
    enrollment: StrategyEnrollmentV2,
    *,
    ...
    if enrollment.execution_mode != EXECUTION_MODE_LIVE or not enrollment.enabled:
        raise ValueError("MTF runtime only executes active LIVE_MANAGE enrollments")
    if enrollment.strategy_key != MTF_LONG_FLAT_STRATEGY_V2:
        raise ValueError("MTF runtime received a non-MTF strategy")
    ensure_mtf_live_schema_v2()
    ...
    if not points:
        raise ValueError("MTF LIVE has no exact canonical 1m history")
    ...
    for timeframe in _TIMEFRAMES:
        ...
        if len(items) < 2:
            raise ValueError(f"MTF LIVE needs enough closed {timeframe}m bars for MACD 12/26/9")
    ...
    state = load_mtf_live_state_v2(enrollment.pilot_key)
    if state is None:
        state = _bootstrap_state_v2(enrollment=enrollment, observed=observed, observations=by_tf)
        return MtfLiveCycleV2(enrollment.pilot_key, state.state, 0, 0, 0, True)
