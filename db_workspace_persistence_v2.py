...
def technical_state_identity_v2(*, market_id: int, as_of: str, technical_recipe_id: UUID) -> UUID:
    return uuid5(PERSISTENCE_NAMESPACE_V2, f"technical-state:{int(market_id)}:{as_of}:{technical_recipe_id}")


def forecast_identity_v2(*, market_id: int, as_of: str, horizon_seconds: int, technical_state_id: UUID, analysis_recipe_id: UUID) -> UUID:
    return uuid5(
        PERSISTENCE_NAMESPACE_V2,
        f"forecast:{int(market_id)}:{as_of}:{int(horizon_seconds)}:{technical_state_id}:{analysis_recipe_id}",
    )


def persist_technical_recipe(*, technical_recipe_id: UUID, name: str, version: int, parameters: dict[str, Any]) -> None:
    with connect() as db:
        json_value = _json_placeholder(db)
        db.execute(
            f"""INSERT INTO pg_v2_technical_recipes
...
def persist_technical_state(*, market_id: int, technical_recipe_id: UUID, state: TechnicalCoreState, technical_state_id: UUID | None = None) -> UUID:
    state_id = technical_state_id or technical_state_identity_v2(
        market_id=market_id, as_of=state.as_of, technical_recipe_id=technical_recipe_id
    )
    features = {
    ...
    with connect() as db:
        json_value = _json_placeholder(db)
        db.execute(
            f"""INSERT INTO pg_v2_technical_states
            ...
            (
                str(state_id), int(market_id), state.as_of, str(technical_recipe_id),
                state.trend_state, state.momentum_state, state.volatility_state, _json(features),
            ),
        )
    return state_id


def persist_analysis_recipe(*, analysis_recipe_id: UUID, name: str, version: int, technical_recipe_id: UUID, enabled_layers: tuple[str, ...], layer_versions: dict[str, str]) -> None:
    """Persist an immutable analysis recipe identity."""
    with connect() as db:
        json_value = _json_placeholder(db)
        db.execute(
            f"""INSERT INTO pg_v2_analysis_recipes
                (analysis_recipe_id, name, version, technical_recipe_id,
                 enabled_layers_json, layer_versions_json)
            VALUES (?, ?, ?, ?, {json_value}, {json_value})
            ON CONFLICT (name, version) DO NOTHING""",
            (
                str(analysis_recipe_id), name, int(version), str(technical_recipe_id),
                _json(list(enabled_layers)), _json(layer_versions),
            ),
        )
...
def persist_baseline_forecast(
    *,
    ...
    frozen_path_spec: dict[str, Any] | None = None,
) -> UUID:
    """Persist an immutable forecast, including the path actually shown when available.

    `frozen_path_spec` is additive inside the existing path_spec_json column, so no
    schema migration is required. Existing historical rows remain valid, while new
    rows can reproduce the exact path/uncertainty interpretation that existed at T0.
    """
    ...
    if frozen_path_spec:
        path_spec.update(dict(frozen_path_spec))
    with connect() as db:
        json_value = _json_placeholder(db)
        db.execute(
            f"""INSERT INTO pg_v2_forecasts
            ...
            (
                str(stored_forecast_id), int(market_id), baseline.as_of, int(baseline.horizon_seconds),
                str(technical_state_id), str(analysis_recipe_id), baseline.expected_return,
                baseline.expected_return, baseline.lower_return, baseline.upper_return, _json(path_spec),
            ),
        )
    return stored_forecast_id


def persist_layer_output(*, market_id: int, as_of: str, output: CachedLayerOutput, layer_output_id: UUID | None = None) -> UUID:
    ...
    with connect() as db:
        json_value = _json_placeholder(db)
        db.execute(
            f"""INSERT INTO pg_v2_forecast_layer_outputs
            ...
            (
                str(stored_id), int(market_id), as_of, output.layer_name, output.layer_version,
                output.input_fingerprint, output.directional_bias, output.velocity_modifier,
                output.uncertainty_modifier, output.reversal_probability, output.squeeze_probability,
                output.confidence, _json(details),
            ),
        )
    return stored_id


def load_cached_layer_outputs(*, market_id: int, workspace: WorkspaceSnapshotV2) -> dict[str, CachedLayerOutput]:
    with connect() as db:
        rows = db.execute(
...
def restore_workspace_layer_cache(*, market_id: int, workspace: WorkspaceSnapshotV2) -> WorkspaceSnapshotV2:
