...
PR #359 (`AutoTrader: isolate P/L reconciliation from live execution`) is merged. CI passed and Railway production deployed successfully at about 07:16 UTC on 2026-09-10.
...
Focused regression tests landed with PR #359 and cover candidate budgeting, backoff fairness/escalation, account-context caching, and the accounting/execution separation contract.
...
1. Inspect STREAM logs during the next real 2m reversal. Success criterion: desired != observed results in CLOSE -> broker FLAT -> OPEN opposite without a reconciliation 429 storm.
2. Verify that background P/L settlement can lag without blocking reversal and that later settlement still updates pilot equity exactly once.
3. If reversal still stalls, inspect strategy CLOSE/live OPEN lifecycle states and Saxo rate-limit responses before changing signal logic.
4. Separately investigate Saxo `AmountLong`/`AmountShort` aggregate semantics; do not modify exact net-direction logic based only on those fields.
5. Once execution is stable over time, return to pending product work: experimental `5m MACD + Price Structure + Impulse Override`, chart/mobile polish, and account-vs-pilot reconciliation display.
