...
_VISIBILITY_KEY = "overview_v2_visible_markets"
...
def _direction_label(direction: str) -> str:
    return {"BULLISH": "BULLISH", "BEARISH": "BEARISH", "NEUTRAL": "NØYTRAL"}.get(str(direction).upper(), str(direction).replace("_", " "))


def _action_label(direction: str) -> str:
    return {"BULLISH": "LONG-BIAS", "BEARISH": "SHORT-BIAS", "NEUTRAL": "AVVENT"}.get(str(direction).upper(), "AVVENT")
    ...
    <article class="pg-market-card pg-market-card-v2" style="--market-color:{html.escape(color, quote=True)}">
      <div class="pg-market-layout pg-market-layout-v2">
        <section class="pg-analysis pg-analysis-v2">
          <div class="pg-state-top"><div class="pg-market"><a class="pg-market-title-link" href="{html.escape(detail_href, quote=True)}" target="_self">{html.escape(view.market)}</a></div><div class="pg-direction">{html.escape(_direction_label(view.direction))}</div></div>
          {explanation}
...
def _subscribed_markets() -> tuple[str, ...]:
    try:
        return tuple(sorted({source.market_name for source in list_subscribed_sources_v2(provider="saxo")}))
    except Exception:
        return ()


def render_v2_overview_market_cards(st, *, asset_color, market_detail_href) -> None:
    """Render subscribed v2 markets; collection and presentation remain separate."""
    ...
    subscribed = _subscribed_markets()
    universe = tuple(sorted(set(subscribed) | set(baselines)))
    if not universe:
        st.info("Venter på aktive canonical v2-markeder.")
        return

    previous = st.session_state.get(_VISIBILITY_KEY)
    if previous is None:
        # Existing and newly onboarded subscribed markets are visible on first load;
        # hiding a card never disables collection.
        default_visible = list(universe)
    else:
        default_visible = [market for market in previous if market in universe]
    with st.expander("Vis markeder", expanded=False):
        visible = st.multiselect(
            "Markeder på Oversikt",
            options=list(universe),
            default=default_visible,
            key=_VISIBILITY_KEY,
            help="Styrer bare hvilke kort som vises. Datainnsamling og analyse fortsetter for subscribed markeder som skjules.",
        )
        st.caption("Visning er separat fra canonical collection subscription.")
    ...
    for market in universe:
        if market not in visible_set:
            continue
        baseline = baselines.get(market)
        if baseline is None:
            st.markdown(
                f'<div class="pg-v2-collecting"><strong>{html.escape(market)}</strong> · COLLECTING DATA<br><span style="opacity:.72">Canonical v2 subscription er aktiv; kortet får Technical Core-prognose når første gyldige workspace er persistert.</span></div>',
        ...
        with st.container():
            ...
            with control_b:
                ...
                selected = st.segmented_control(f"Prognosehorisont · {market}", options=baseline.available_horizons, default=preferred, format_func=_horizon_label, key=f"{_HORIZON_PREFIX}{market}", label_visibility="collapsed")
                selected_horizons[market] = int(selected if selected is not None else preferred)
            with control_c:
                enabled = st.checkbox("Technical Interpreter", value=False, disabled=not baseline.interpreter_available, help="Komponerer fingerprint-matchet cached output; ingen AI-kall fra Overview." if baseline.interpreter_available else "Ingen kompatibel cached Interpreter-output for dette workspace-snapshotet.", key=f"{_INTERPRETER_PREFIX}{market}")
    ...
    if not placeholders:
        return
    try:
        views = load_v2_overview_snapshots(requested_horizons=selected_horizons, interpreter_by_market=interpreter_by_market)
    except Exception as exc:
        st.warning(f"Kunne ikke komponere valgte v2-visninger: {exc}")
        return
    try:
        persisted_health = {item.stage: item for item in load_runtime_health_v2(service="v2-technical-runtime")}
    except Exception:
        persisted_health = {}
    for market, placeholder in placeholders.items():
        ...
        placeholder.markdown(render_overview_v2_market_card_html(view, health=health, color=asset_color(market), detail_href=market_detail_href(market)), unsafe_allow_html=True)
    st.caption("Markedskortene leser persistert v2 Technical Core/workspace. Visningsvalg påvirker ikke canonical datainnsamling.")
