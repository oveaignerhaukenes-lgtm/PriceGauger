...
from trading_desk import TIMEFRAME_MINUTES, normalized_close_series, resample_bars
...
available_markets = [
    market for market in configured_markets if store.load_latest_bar(market=market) is not None
]
...
def _load(name: str):
    raw = store.load_range(market=name, start=start, end=now, limit=10000)
    return resample_bars(raw, timeframe=timeframe)


try:
    primary = _load(market)
except ValueError as exc:
    st.error(f"Ugyldig canonical barserie for {market}: {exc}")
    st.stop()

if not primary:
    st.info(f"Ingen ferdige 1m-bars for {market} i valgt tidsvindu.")
    st.stop()
...
for overlay_market in overlays:
    try:
        overlay_bars = _load(overlay_market)
    except ValueError as exc:
        st.warning(f"Hopper over {overlay_market}: {exc}")
        continue
    if not overlay_bars:
        st.warning(f"Ingen bars for {overlay_market} i valgt tidsvindu.")
        continue
