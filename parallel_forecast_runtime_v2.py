...
def load_unresolved_parallel_experiments_v2(*, db_path: str = "pricegauger.db", limit: int = 500) -> tuple[ParallelForecastExperimentV2, ...]:
...
def run_parallel_forecast_runtime_cycle_v2(
    ...
    *,
    ...
    attempted, inserted = record_parallel_experiments_v2(produced, db_path=db_path, scope_key=scope_key)
    resolved = resolve_parallel_outcomes_v2(history_store=history_store, db_path=db_path)
    return ParallelForecastRuntimeSummaryV2(
        experiments_attempted=attempted,
        experiments_inserted=inserted,
