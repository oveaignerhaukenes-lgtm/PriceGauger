...
def test_2m_5m_and_15m_shadow_keys_are_the_same_keys_exposed_live() -> None:
    ...
    assert LIVE_INTRABAR_MACD_TIMEFRAMES_V1 == (2, 5)
...
def test_2m_and_5m_use_persisted_forming_candle_clock_but_15m_stays_closed_bar() -> None:
    ...
    assert "if minutes in LIVE_INTRABAR_MACD_TIMEFRAMES_V1" in runtime
    assert "live_macd_intrabar_clock_v1(" in runtime
    assert "else:\n        clock = _timeframe_clock_v1" in runtime
