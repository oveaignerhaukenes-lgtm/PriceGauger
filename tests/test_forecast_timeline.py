...
def test_timeline_keeps_multiple_forecasts_and_actual_price():
    ...
    svg = render_forecast_timeline_svg(forecasts, observed_prices=observed, now=datetime(2026, 8, 10, 0, 40, tzinfo=timezone.utc))
    assert "2 AKTIVE SPOR" in svg
...
def test_weekend_gap_is_compressed_and_actual_price_is_not_connected_across_it():
    ...
    observed_dt = ((friday_a, 4198.0), (friday_b, 4200.0), (sunday_a, 4220.0), (sunday_b, 4222.0))
    ...
    forecast = _forecast(suffix="weekend", as_of="2026-08-09T22:00:00+00:00", low=-0.2, high=0.4)
    svg = render_forecast_timeline_svg((forecast,), observed_prices=tuple((stamp.isoformat(), price) for stamp, price in observed_dt), now=sunday_b)
    # Long-history mode deliberately retains both sides of the weekend, but the
    # realized line must remain split rather than bridging the closed market.
...
def test_narrow_compressed_gap_does_not_render_cramped_ufo_label():
    forecast = _forecast(suffix="gap", as_of="2026-08-10T08:00:00+00:00", low=-0.2, high=0.4)
    observed = (("2026-08-10T00:00:00+00:00", 4200.0), ("2026-08-10T00:01:00+00:00", 4201.0), ("2026-08-10T08:00:00+00:00", 4210.0), ("2026-08-10T08:01:00+00:00", 4211.0))
...
def test_large_non_weekend_data_gap_is_labeled_market_gap():
    observed = ((datetime(2026, 8, 10, 0, 0, tzinfo=timezone.utc), 4200.0), (datetime(2026, 8, 10, 8, 0, tzinfo=timezone.utc), 4210.0))
    gaps = _timeline_gaps(observed)
    assert len(gaps) == 1
    assert gaps[0].label == "MARKET GAP"


def test_timeline_default_keeps_all_forecasts_that_overlap_recent_forecast_trail():
    start = datetime(2026, 8, 10, 0, 0, tzinfo=timezone.utc)
    forecasts = tuple(_forecast(suffix=str(index), as_of=(start + timedelta(minutes=15 * index)).isoformat(), low=0.1 + 0.01 * index, high=0.5 + 0.01 * index) for index in range(14))
    svg = render_forecast_timeline_svg(forecasts)
    assert "14 AKTIVE SPOR" in svg
...
def test_timeline_explicit_layer_limit_still_keeps_newest():
    forecasts = tuple(_forecast(suffix=str(index), as_of=f"2026-08-10T0{index}:00:00+00:00", low=0.1 * index, high=0.5 + 0.1 * index) for index in range(5))
    svg = render_forecast_timeline_svg(forecasts, max_layers=4)
    assert "4 AKTIVE SPOR" in svg
    assert svg.count("pg-forecast-fan") == 4
    assert svg.count("pg-forecast-base") == 4


def test_forecast_trails_are_bounded_relative_to_latest_forecast_origin():
    forecasts = (
    ...
    observed = (("2026-08-10T11:59:00+00:00", 4210.0), ("2026-08-10T12:00:00+00:00", 4211.0))
    svg = render_forecast_timeline_svg(forecasts, observed_prices=observed)
    # Long chart history is independent of forecast-trail retention. The recent
    # trail starts two horizons before the latest 08:00 origin, i.e. at 00:00;
    # all three forecasts still overlap that bounded trail.
    assert "3 AKTIVE SPOR" in svg
    assert svg.count("pg-forecast-fan") == 3
    assert svg.count("pg-forecast-base") == 3


def test_latest_forecast_remains_visible_when_realized_history_advances_beyond_it():
    forecast = _forecast(suffix="old", as_of="2026-08-10T00:00:00+00:00", low=0.1, high=0.5)
    observed = (("2026-08-10T12:59:00+00:00", 4210.0), ("2026-08-10T13:00:00+00:00", 4211.0))
    svg = render_forecast_timeline_svg((forecast,), observed_prices=observed)
    # The latest immutable forecast is retained as comparison context even when
    # realized long-history data has advanced beyond its horizon.
    assert "1 AKTIVE SPOR" in svg
    assert svg.count("pg-forecast-fan") == 1
    assert svg.count("pg-forecast-base") == 1
    assert "pg-realized" in svg


def test_incomplete_forecast_is_not_drawn_as_complete_layer():
    complete = _forecast(suffix="ok", as_of="2026-08-10T00:00:00+00:00", low=0.2, high=1.0)
    incomplete = ForecastSnapshot(
        forecast_id="forecast:bad", market="Gold", as_of="2026-08-10T00:30:00+00:00", reference_price=4200.0,
        direction="LONG_BIAS", direction_score=0.7, confidence=0.65, expected_move_low_pct=None,
        expected_move_high_pct=None, horizon_hours=4.0, time_scale="HOURS", decision_snapshot_id="decision:bad",
        information_snapshot_id="information:bad", market_snapshot_id="market:bad", status="DEGRADED",
        missing_inputs=("expected_move_interval",), status_reason="test",
    )
