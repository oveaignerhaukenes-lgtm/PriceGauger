...
def _load_holistic_decisions(instrument_id: int, start: datetime, end: datetime) -> tuple[dict, ...]:
    ...
    result=[]
    for row in rows:
        values=dict(row) if isinstance(row,dict) else {"action_at":row[0],"price":row[1],"target_direction":row[2],"confidence":row[3],"summary":row[4]}
        word=str(values["target_direction"]).upper(); target=1 if word=="LONG" else -1 if word=="SHORT" else 0
        result.append({"at":_stamp(values["action_at"]),"price":float(values["price"]),"target":target,"confidence":float(values["confidence"]),"summary":str(values["summary"])})
    return tuple(result)

def _target_frame(price_frame: pd.DataFrame, decisions: tuple[dict,...]) -> pd.DataFrame:
    frame=price_frame[["PRICE"]].copy(); target=pd.Series(0.0,index=frame.index,dtype="float64")
    ordered=sorted(decisions,key=lambda item:item["at"]); current=cursor=0
    for index,at in enumerate(frame.index):
        ...
        while cursor<len(ordered) and ordered[cursor]["at"]<=stamp:
            ...
            if proposed in (-1,0,1): current=proposed
            cursor+=1
        target.iloc[index]=current
    frame["TARGET"]=target; return frame

def _events_from_target(frame: pd.DataFrame) -> tuple[dict,...]:
    events=[]; previous=0
    for at,row in frame.iterrows():
        current=int(row["TARGET"])
        if current!=previous and current in (-1,1): events.append({"at":_stamp(at),"price":float(row["PRICE"]),"target":current})
        previous=current
    return tuple(events)
...
def _render_metrics(name: str, frame: pd.DataFrame, *, cost_bps: float) -> None:
    ...
    st.markdown(f"**{name}**"); cols=st.columns(5)
    cols[0].metric("Treff",f"{summary.win_rate_pct:.1f}%"); cols[1].metric("Retur",f"{summary.return_pct:+.1f}%"); cols[2].metric("Capture",f"{summary.capture_area_pct:.1f}%"); cols[3].metric("Max DD",f"{summary.max_drawdown_pct:.1f}%"); cols[4].metric("Skift",str(summary.switches))

def render_tradingdesk_three_trader_lab_v1(context: TradingDeskV2Context) -> None:
    instrument_id=context.instrument_id
    if instrument_id is None: return
    with st.container(border=True):
        st.markdown("### Tre tradere · samme marked")
        st.caption("Sammenligner tre uavhengige spor på samme canonical prisserie: enkel MACD-supervisor, adaptiv MACD-periode og den bredere holistiske AI-baselinen. Analyse-only; ingen av disse markørene sender ordre.")
        controls=st.columns([1,1,2])
        with controls[0]: hours=int(st.selectbox("Vindu",(6,12,24,48),index=2,format_func=lambda value:f"{value} t",key=f"three-trader-window-{instrument_id}"))
        with controls[1]: cost_bps=float(st.number_input("Kostnad / ordrebein (bps)",0.0,25.0,0.0,0.1,key=f"three-trader-cost-{instrument_id}"))
        with controls[2]: visible=st.multiselect("Vis / plott",ALL_MODELS,default=ALL_MODELS,key=f"three-trader-visible-{instrument_id}")
        end=datetime.now(timezone.utc); start=end-timedelta(hours=hours+18)
        try:
        ...
        if HOLISTIC_NAME in visible:
            if decisions: _render_metrics(HOLISTIC_NAME,holistic_frame,cost_bps=cost_bps)
            else: st.markdown(f"**{HOLISTIC_NAME}**"); st.caption("Ingen lagrede holistiske AI-beslutninger i valgt vindu ennå.")
        render_three_trader_tv_v1(rule_frame,event_sets,visible,key=f"three-trader-chart-{instrument_id}")
        st.caption("TV-chart: dra/pinch/zoom i grafen, dra håndtaket under for høyde. Visningsområde og høyde huskes. Rule / Adaptiv / AI-markører viser modellskiftene på samme prisserie.")
