...
_LIGHTWEIGHT_CHARTS_VERSION = "5.2.1"
...
_LIGHTWEIGHT_LIVE_JS = rf"""
const LIB_URL = {_LIGHTWEIGHT_CHARTS_URL!r};
const LIB_VERSION = {_LIGHTWEIGHT_CHARTS_VERSION!r};
...
export default function(component) {{
    ...
    function loadLibrary() {{
        ...
        window.__pricegaugerLightweightChartsPromise = new Promise((resolve, reject) => {{
            ...
            script.src = LIB_URL;
            script.async = true;
            script.dataset.pgLightweightCharts = LIB_VERSION;
    ...
    function theme() {{
        ...
        return dark ? {{
            ...
            legendBg: 'rgba(14,17,23,.82)',
            legendBorder: 'rgba(148,163,184,.22)',
        }} : {{
            background: '#ffffff',
            text: '#374151',
            grid: 'rgba(15,23,42,.10)',
            border: 'rgba(15,23,42,.24)',
            legendBg: 'rgba(255,255,255,.88)',
            legendBorder: 'rgba(15,23,42,.14)',
        }};
    }}
    ...
    function createRoot() {{
        ...
        root.className = 'pg-lightweight-live-chart';
        Object.assign(root.style, {{
            width: '100%', height: '100%', minWidth: '0', position: 'relative',
        }});
        parentElement.appendChild(root);

        const legend = document.createElement('div');
        legend.className = 'pg-lightweight-legend';
    ...
    function paneIndex(payload, paneName) {{
        const index = Array.from(payload.pane_order || ['price']).indexOf(paneName);
        return index < 0 ? 0 : index;
    }}

    function seriesOptions(item, style) {{
        const options = {{
            color: style.color,
            lineWidth: style.lineWidth,
            priceLineVisible: false,
            lastValueVisible: item.pane !== 'price' || !String(item.role || '').startsWith('bollinger_'),
    ...
    function buildChart(LWC, previousVisibleRange = null) {{
        const colors = theme();
        const {{ root, legend }} = createRoot();
        legend.style.background = colors.legendBg;
        legend.style.border = `1px solid ${{colors.legendBorder}}`;
        legend.style.color = colors.text;
        legend.textContent = `${{payload.market || ''}} · ${{payload.timeframe || ''}}`;
        ...
        candles.setData(Array.from(payload.candles || []));

        const series = new Map();
        const labels = new Map();
        series.set('candles', candles);
        labels.set(candles, String(payload.market || 'Pris'));
        ...
        Array.from(payload.lines || []).forEach((item, index) => {{
            ...
            const pane = paneIndex(payload, item.pane);
            const line = chart.addSeries(LWC.LineSeries, seriesOptions(item, style), pane);
            line.setData(Array.from(item.data || []));
            series.set(String(item.role), line);
            labels.set(line, String(item.label || item.role || 'Serie'));
        }});

        Array.from(payload.histograms || []).forEach((item, index) => {{
            const pane = paneIndex(payload, item.pane);
            const histogram = chart.addSeries(LWC.HistogramSeries, {{
                title: String(item.label || ''),
                color: 'rgba(124,58,237,.32)',
                priceLineVisible: false,
                lastValueVisible: false,
            }}, pane);
            const values = Array.from(item.data || []).map((point) => ({{
                time: point.time,
                value: point.value,
        ...
        if (rsi) {{
            for (const threshold of Array.from(payload.thresholds?.rsi || [])) {{
                rsi.createPriceLine({{
                    price: Number(threshold), color: 'rgba(100,116,139,.55)',
                    lineWidth: 1, lineStyle: LWC.LineStyle.Dashed,
                    axisLabelVisible: false, title: '',
                }});
            }}
            rsi.priceScale().applyOptions({{ autoScale: true, scaleMargins: {{ top: .08, bottom: .08 }} }});
        }}
        ...
        if (LWC.createSeriesMarkers) {{
            markers = LWC.createSeriesMarkers(candles, Array.from(payload.markers || []), {{
                autoScale: false,
                zOrder: 'top',
            }});
        }}
        ...
        chart.subscribeCrosshairMove((param) => {{
            ...
            legend.textContent = parts.length ? parts.join(' · ') : `${{payload.market || ''}} · ${{payload.timeframe || ''}}`;
        }});
    ...
    function updateEntry(entry) {{
        ...
        entry.candles.setData(Array.from(payload.candles || []));
        ...
        for (const item of Array.from(payload.histograms || [])) {{
            ...
            entry.series.get(String(item.role))?.setData?.(values);
        }}
...
_lightweight_live_component = st.components.v2.component(
    "pricegauger_tradingdesk_lightweight_live_v1",
    js=_LIGHTWEIGHT_LIVE_JS,
    isolate_styles=False,
)


def render_lightweight_live_chart_v1(
    payload: Mapping[str, Any],
    *,
    key: str,
) -> None:
    """Render TradingDesk LIVE data with native Lightweight Charts interactions."""

    height = max(320, int(payload.get("height", 780)))
    _lightweight_live_component(
        key=str(key),
...
__all__ = ["render_lightweight_live_chart_v1"]
