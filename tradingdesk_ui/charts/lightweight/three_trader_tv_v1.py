...
export default function(component) {
    ...
    script.onload = () => {
        ...
        const chart = L.createChart(root, {
            ...
            timeScale: { borderVisible: true, timeVisible: true, secondsVisible: false, rightOffset: 3 },
        ...
        for (const event of (payload.events || [])) {
            markers.push({
                time: event.time,
                position: event.target > 0 ? 'belowBar' : 'aboveBar',
                shape: event.target > 0 ? 'arrowUp' : 'arrowDown',
                color: event.color,
                text: event.short,
                size: event.size || 1,
            });
        ...
        if (L.createSeriesMarkers) L.createSeriesMarkers(price, markers);
        if (saved.range) { try { chart.timeScale().setVisibleLogicalRange(saved.range); } catch (_) {} }
    ...
    handle.title = 'Dra for å endre høyden';
    ...
    Object.assign(grip.style, {width:'46px', height:'3px', borderRadius:'99px', background:'#cbd5e1'});
    handle.appendChild(grip); shell.appendChild(handle);
    handle.addEventListener('pointerdown', event => {
        event.preventDefault(); handle.setPointerCapture?.(event.pointerId);
        const startY = event.clientY, startHeight = root.getBoundingClientRect().height;
...
_three_trader_tv_component = st.components.v2.component(
    "pricegauger_three_trader_tv_v1", js=_THREE_TRADER_TV_JS, isolate_styles=False
)
...
def render_three_trader_tv_v1(price_frame, event_sets, visible_models, *, key: str) -> None:
    ...
    shorts = {"Dum MACD": "Rule", "MACD-adaptiv": "Adaptiv", "Holistisk AI": "AI"}
    sizes = {"Dum MACD": 1, "MACD-adaptiv": 2, "Holistisk AI": 3}
    events = []
    for model, model_events in event_sets.items():
        ...
        for event in model_events:
            events.append({
                "time": int(event["at"].timestamp()), "target": int(event["target"]),
                "color": colors.get(model, "#64748b"), "short": shorts.get(model, model),
