...
def _latest_price(market: pd.DataFrame) -> float:
    closes = pd.to_numeric(market.get("close"), errors="coerce").dropna()
    if closes.empty:
...
def _realized_volatility_pct(market: pd.DataFrame, lookback: int = 48) -> float:
    closes = pd.to_numeric(market.get("close"), errors="coerce").dropna().tail(lookback + 1)
    returns = closes.pct_change().dropna() * 100.0
    if len(returns) < 3:
        return 0.35
    return max(0.05, min(4.0, float(returns.std(ddof=0))))
...
def _direction_sign(direction: str) -> int:
    ...
    return 1 if normalized == "LONG" else -1 if normalized == "SHORT" else 0
...
def build_trajectory(
    *,
    ...
    """Build a transparent forecast cone that widens with time and uncertainty."""
    horizons = tuple(int(value) for value in horizons_minutes)
    confidence = _bounded(confidence_pct / 100.0, 0.0, 0.95)
    uncertainty_multiplier = 1.15 - 0.75 * confidence
    maximum = max(max(horizons), 1)
    points: list[TrajectoryPoint] = []

    for minutes in horizons:
        progress = 0.0 if minutes <= 0 else 1.0 - 0.5 ** (minutes / max(maximum / 2.0, 1.0))
        centre = expected_move_pct * progress
        uncertainty = volatility_pct * sqrt(max(minutes, 1) / 15.0) * uncertainty_multiplier
        points.append(
            TrajectoryPoint(
...
def build_trade_plan(
    *,
    ...
    if sign > 0:
        centre = _price(reference, -pullback_pct)
        entry_low = _price(centre, -entry_width_pct / 2.0)
        entry_high = _price(centre, entry_width_pct / 2.0)
    ...
    if response_role:
        reasons.insert(0, f"Responsrolle: {response_role}.")
...
def update_plan_status(plan: TradePlan, current_price: float) -> str:
    """Evaluate lifecycle state while preserving the original forecast."""
    ...
    if plan.direction == "LONG":
        ...
        if plan.entry_low <= current_price <= plan.entry_high:
            return "ENTRY_ZONE"
        return "WAITING_FOR_PULLBACK" if current_price > plan.entry_high else "BELOW_ENTRY_ZONE"
    ...
    if plan.entry_low <= current_price <= plan.entry_high:
        return "ENTRY_ZONE"
    return "WAITING_FOR_PULLBACK" if current_price < plan.entry_low else "ABOVE_ENTRY_ZONE"
